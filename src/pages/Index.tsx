      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              className="w-full h-40 px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none font-mono text-sm"
          ...
          <button
            ...
            onClick={handleCheck}
            disabled={isChecking}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-semibold py-3 px-6 rounded-lg transition-all flex items-center justify-center gap-2 shadow-lg hover:shadow-xl"
          >
            {isChecking ? (
              <>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl shadow-xl p-6">
            ...
            <div
              id="list_live"
              className="bg-green-50 rounded-lg p-4 h-96 overflow-y-auto border-2 border-green-200"
            >
              {liveUsers.length === 0 ? (
                <p className="text-gray-400 text-center py-8">暫無活躍用戶</p>
              ) : (
                <div className="space-y-2">
                  {liveUsers.map((uid, index) => (
                    <div
                      key={index}
                      className="bg-white px-3 py-2 rounded border border-green-300 font-mono text-sm"
                    >
                      {uid}
                    </div>
                  ))}
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6">
            ...
            <div
              id="list_die"
              className="bg-red-50 rounded-lg p-4 h-96 overflow-y-auto border-2 border-red-200"
            >
              {dieUsers.length === 0 ? (
                <p className="text-gray-400 text-center py-8">暫無非活躍用戶</p>
              ) : (
                <div className="space-y-2">
                  {dieUsers.map((uid, index) => (
                    <div
                      key={index}
                      className="bg-white px-3 py-2 rounded border border-red-300 font-mono text-sm"
                    >
                      {uid}
                    </div>
                  ))}
