...
// 引入Lucide React图标库，确保你已经安装了 `lucide-react`
import {
  Check,
  CheckCircle,
  Copy,
...
const UI_UPDATE_INTERVAL = 500; // UI刷新间隔
const VIRTUAL_ITEM_HEIGHT = 48; // 虚拟列表项高度
const VIRTUAL_BUFFER = 5; // 虚拟列表缓冲区
...
const App: React.FC = () => {
    ...
    const checkSingleUser = useCallback(async (id: string, maxRetries = 2): Promise<UserResult> => {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                const res = await fetch(url, { keepalive: true } as RequestInit);
                ...
                const status: UserResult['status'] = urlField.includes('static.xx.fbcdn.net') ? 'Die' : 'Live';
        ...
        return { id, status: 'Die', url: '' }; // Fallback
    }, []);
    ...
    const handleCopy = useCallback(async (text: string, onSuccess: () => void) => {
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white font-sans pb-24 selection:bg-blue-500/30">
            <div className="mx-auto flex max-w-7xl flex-col gap-4 px-3 py-4 sm:gap-6 sm:px-6 sm:py-12">
                    ...
                    <main className="flex flex-col gap-6">
                        ...
                        <section className="rounded-3xl bg-white/5 p-4 sm:p-6 border border-white/10 backdrop-blur-md">
                            <h2 className="text-base sm:text-lg font-semibold text-white flex items-center gap-2 mb-3">
                                <Edit2 className="h-4 w-4 text-blue-400"/> 输入账号ID
                            </h2>
                            <textarea
                                className="min-h-[150px] w-full rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white placeholder-slate-500 outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all font-mono resize-vertical"
                                value={inputValue}
                                onChange={(e) => setInputValue(e.target.value)}
                                placeholder={`一行一个ID，例如:\n100001234567890\n100002345678901`}
                                disabled={state.isChecking}
                            />
                            <div className="mt-4">
                                <button
                                    onClick={handleStartCheck}
                                    disabled={state.isChecking || !inputValue.trim()}
                                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3.5 text-base font-bold text-white shadow-lg shadow-blue-600/20 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {state.isChecking ? <Loader2 className="h-5 w-5 animate-spin"/> : <Play className="h-5 w-5 fill-current" />}
                                    {state.isChecking ? `检查中... (${progressPercentage}%)` : '开始检查'}
                                </button>
                            </div>
                    ...
                    <main className="space-y-4 sm:space-y-6">
                        <div className="flex items-center justify-between px-1">
                            ...
                            {historyRecords.length > 0 && (
                                <button 
                                    onClick={() => { if(window.confirm('确定清空所有历史记录？此操作不可恢复。')) { localStorage.clear(); setHistoryRecords([]); } }}
                        ...
                        <div className="grid gap-3 sm:gap-4">
                            {historyRecords.map(record => (
                                <div key={record.key} className="rounded-2xl bg-white/5 border border-white/10 p-4 transition-colors">
                                    <div className="flex flex-col gap-3">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <div className="text-xs text-slate-400 mb-1">
                                                    {new Date(record.timestamp).toLocaleString(undefined, { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                </div>
                                                <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => handleRecheckHistory(record)} className="p-2 bg-white/5 rounded-lg active:bg-white/20 text-slate-300" title="重新检查">
                                                    <RefreshCw className="h-4 w-4"/>
                                                </button>
                                                <button onClick={() => handleCopy(record.users.map(u => u.id).join('\n'), () => setCopiedHistoryKey(record.key))} className="p-2 bg-white/5 rounded-lg active:bg-white/20 text-slate-300" title="复制所有ID">
                                                    {copiedHistoryKey === record.key ? <Check className="h-4 w-4 text-emerald-400"/> : <Copy className="h-4 w-4"/>}
                                                </button>
                                                <button onClick={() => setExpandedRecords(p => ({...p, [record.key]: !p[record.key]}))} className="p-2 bg-white/5 rounded-lg active:bg-white/20 text-slate-300" title="查看详情">
                                        ...
                                        {editingNoteKey === record.key ? (
                                            <div className="flex gap-2">
                                                <input 
                                                    className="flex-1 bg-black/30 rounded px-3 py-2 text-sm text-white border border-white/10 outline-none focus:border-blue-500"
                                                ...
                                                <button onClick={() => {
                                                    ...
                                                    const recordToStore = { ...newRec };
                                                    delete (recordToStore as any).key; // 不在localStorage中存key
                                                    localStorage.setItem(record.key, JSON.stringify(recordToStore));
                                                    setHistoryRecords(p => p.map(x => x.key === record.key ? newRec : x));
                                                    setEditingNoteKey(null);
                                                }} className="text-sm bg-blue-600 px-4 rounded-lg text-white font-bold">保存</button>
                                            </div>
                                        ) : (
                                            <div className="text-sm text-slate-300 flex items-center gap-2 py-2 px-3 rounded-lg bg-white/5 cursor-pointer hover:bg-white/10" onClick={() => { setEditingNoteKey(record.key); setNoteDraft(record.note); }}>
                                        ...
                                        {expandedRecords[record.key] && (
                                            <div className="mt-2 pt-3 border-t border-white/10 grid grid-cols-1 md:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
                                                {record.users.slice(0, 100).map(u => (
                                                    <div key={u.id} className={`text-xs px-2 py-1 rounded flex items-center justify-between ${u.status === 'Live' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                                                        <span className="font-mono">{u.id}</span>
                                                        <span className="font-bold">{u.status}</span>
                                                    </div>
                                                ))}
                                                {record.users.length > 100 && <div className="col-span-full text-center text-xs text-slate-500 mt-2">仅显示前 100 条记录</div>}
                                            </div>
                                        )}
                                    </div>
                                </div>
                ...
                {showSavePrompt && state.results.length > 0 && (
                    <div className="fixed bottom-0 left-0 right-0 p-4 z-50 bg-gradient-to-t from-slate-900 to-slate-900/90 backdrop-blur-lg border-t border-white/10 animate-in slide-in-from-bottom duration-300">
                        <div className="mx-auto max-w-7xl flex flex-col gap-3">
                            <div className="flex items-center justify-between text-white">
                                ...
                                <span className="text-xs opacity-70">Live: {liveResults.length} / Die: {dieResults.length}</span>
                            </div>
                            <div className="flex gap-2">
                                <input 
                                    type="text" 
                                ...
                                <button 
                                    ...
                                    className="text-white/60 px-2 text-sm hover:text-white"
                                >
                                    放弃
                                </button>
                            </div>
