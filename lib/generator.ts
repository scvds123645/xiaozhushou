...
const phoneSegments: Record<string, string[]> = {  
  ...
  US: [  
    ...
    '602', '603', '605', '606', '607', '608', '609', '610', '614', '615', '616', '617', '618', '619', '620', // 6开头  
...
function convertToLatinChars(name: string): string {  
  ...
  // 如果已经是英文,直接返回小写并移除特殊字符(除了空格和连字符，这些稍后处理)  
...
export function generateEmail(firstName: string, lastName: string) {  
  ...
  const shortYear = birthYear.toString().slice(-2); // 例如: 95  
  const currentYear = new Date().getFullYear();  
