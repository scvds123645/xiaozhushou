...
// 辅助函数：将字符转换为拉丁字母 (处理非ASCII字符，简单回退策略)  
function convertToLatinChars(str: string): string {  
  ...
  // 3. 如果结果为空（例如全是中文），生成一个随机的短字符串作为回退，防止邮箱生成失败  
  if (ascii.length === 0) {  
...
export function generateEmail(firstName: string, lastName: string) {    
  ...
  // 如果你想用回完整的超长列表，请把你原来的 domains 数组复制回来覆盖这里  
  // 为了构建通过，至少需要保证 convertToLatinChars 存在  
  ...
  return `$${username}@$${domain}`; // 注意：移除了原本代码中可能导致错误的额外 $ 符号  
}  
